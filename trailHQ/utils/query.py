...
queryList= {
    ...
    'STsing': "select s.name, s.difficulty, s.description, s.length, s.rating, s.url  from trailHQ_matches m left join trailHQ_singletrackstrail s on s.key = m.singleTracksId_id where m.singleTracksId_id = %s",
    'TFsing': "select tft.description as TFTDesc, tfi.name as TFName, tft.difficulty as TFTDiff, tft.lastReport as TFTRepor, tft.length as TFTLength, tft.climb as TFTClimb, tft.descent as TFTDescent, tft.area as TFTArea, tft.trailType as TFTType, tfi.url as URL from trailHQ_matches m left join trailHQ_tftrail tft on tft.Tid_id = m.TfTrailId_id left join trailHQ_tfid tfi on tfi.Tid = tft.Tid_id where m.singleTracksId_id = %s",
    'TFAsing': "select tfa.regionDesc as TFADesc, tfa.regionDiff as TFADiff, tfa.localTrailGroup as TFAGroup, tfa.length as TFALen, tfa.vertical as TFAVert, tfa.numTrails as TFANum, tfai.riding_area as Riding_Area from trailHQ_matches m join trailHQ_tfarea tfa on tfa.Aid_id = m.TfAreaId_id join trailHQ_tfstatearea tfai on tfai.Aid = tfa.Aid_id where m.singleTracksId_id = %s",
    'MBsing': "select mpt.description as MDesc, mpi.name as MName, mpt.orgs as MORgs, mpt.length as MLen, mpt.ascent as MAscent, mpt.descent as MDescent, mpt.highElev as MHelev, mpt.lowElev as MLelev, mpt.avgGrade as MAGr, mpt.maxGrade as MMGr from trailHQ_matches m join trailHQ_mtbprojtr mpt on mpt.Mid_id = m.MTrailId_id join trailHQ_mtbprojtrailid mpi on mpi.mtrailId = mpt.Mid_id where m.singleTracksId_id = %s",
}
